...
public class CountrySearchServiceTest {
	...
	Country country;
	    ...
	    public void countryBatchImport() throws JsonParseException,
	        ...
	        doNothing().when(countryDao).batchUpdate(providerCapabilities.get(com.imi.rest.constants.ProviderConstants.TWILIO),
	        		providerTwilio);
	        doNothing().when(countryDao).batchUpdate(providerCapabilities.get(com.imi.rest.constants.ProviderConstants.NEXMO),
	        		providerNexmo);
	        doNothing().when(countryDao).batchUpdate(providerCapabilities.get(com.imi.rest.constants.ProviderConstants.PLIVO),
	        		providerPlivo);
	    }
}
