...
public class ExceptionMapper {
	...
	public ApiError handleException(HttpServletResponse response, Exception ex) {
		...
		if (ex instanceof InboundRestException) {
			...
			HttpStatus status = apiException.getStatus() != null ? apiException.getStatus()
					: HttpStatus.INTERNAL_SERVER_ERROR;
			int errorCode = apiException.getCode() != 0 ? apiException.getCode() : DEFAULT_ERROR_CODE;
			apiError = ApiError.createApiError(status, errorCode, apiException.getMessage(),
					apiException.getDetailedMessage());
		} else {
			apiError = ApiError.createApiError(HttpStatus.INTERNAL_SERVER_ERROR, DEFAULT_ERROR_CODE, "Error happened",
					ex.getMessage());
		}
