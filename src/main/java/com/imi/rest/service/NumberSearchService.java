...
public class NumberSearchService implements ProviderConstants {
	...
	public NumberResponse searchPhoneNumbers(ServiceConstants serviceTypeEnum, Provider provider, String countryIsoCode,
			String numberType, String pattern, String nextPlivoIndex, String nextNexmoIndex, String markup)
		...
		if (provider.getName().equalsIgnoreCase(PLIVO)) {
			plivoFactoryImpl.searchPhoneNumbers(providerService.getPlivioProvider(), serviceTypeEnum, countryIsoCode,
					numberType, pattern, nextPlivoIndex, numberResponse, markup);
		} else if (provider.getName().equalsIgnoreCase(TWILIO)) {
			twilioFactoryImpl.searchPhoneNumbers(providerService.getTwilioProvider(), serviceTypeEnum, countryIsoCode,
					numberType, pattern, twiloIndex, numberResponse, markup);
		} else if (provider.getName().equalsIgnoreCase(NEXMO)) {
			nexmoFactoryImpl.searchPhoneNumbers(providerService.getNexmoProvider(), serviceTypeEnum, countryIsoCode,
					numberType, pattern, nextNexmoIndex, numberResponse, markup);
		} else {
	...
	public NumberResponse searchPhoneNumbers(ServiceConstants serviceTypeEnum, String countryIsoCode, String numberType,
			String pattern, String nextPlivoIndex, String nextNexmoIndex, String markup)
					throws ClientProtocolException, IOException {
		NumberResponse numberResponse = new NumberResponse();
		String twiloIndex = "";
		if ("FIRST".equalsIgnoreCase(nextNexmoIndex) || "FIRST".equalsIgnoreCase(nextPlivoIndex)
