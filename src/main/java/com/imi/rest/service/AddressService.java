...
public class AddressService implements ProviderConstants {
	...
	public List<Address> getAddressByCustomerName(String customerName, String providerName, Integer clientId)
			throws ClientProtocolException, IOException {
		Provider provider = providerService.getProviderByName(providerName);
		SubAccountDetails customerAccountDetails = twilioFactoryImpl.getSubAccountDetails("" + clientId, provider);
		List<Address> addressList = null;
		if (provider.getName().equalsIgnoreCase(TWILIO)) {
			addressList = twilioFactoryImpl.getAddressOfCustomer(customerName, provider, clientId,
					customerAccountDetails);
		}
		return addressList;
	}

	public void updateClientAddressToProvider(Customer customer, String providerName, Integer clientId)
			throws ClientProtocolException, IOException {
		Provider provider = providerService.getProviderByName(providerName);
		if (providerName.equalsIgnoreCase(TWILIO)) {
			twilioFactoryImpl.createNewAddressOfCustomer(customer, provider, clientId);
		} else {
			throw InboundRestException.createApiException(InboundApiErrorCodes.INVALID_PROVIDER_ACTION_EXCEPTION,
					"Provider " + providerName + "does not support updating address through api");
		}
	}
	...
	public List<Customer> getAddressList(Integer userid, String country) {
		return addressDao.getAddressList(userid, country == null ? null : country.toUpperCase());
	}
