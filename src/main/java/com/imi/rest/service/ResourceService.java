...
public class ResourceService {
    ...
    public ResourceMaster updateResource(String number,
            ...
            ServiceConstants serviceTypeEnum, Provider provider,
            Country country, Integer userid, Integer clientId, Integer groupid,
            Integer teamid, String clientname, String clientkey,
            String teamuuid, String numberType) {
        ResourceMaster resourceMaster = new ResourceMaster();
        try {
            resourceMaster.setServiceCode(number);
            // may need to configure
            resourceMaster.setChannel(ImiDataFormatUtils
                    .getChannel(serviceTypeEnum));
            List<Object> creditsResultList = defaultQueryDao
                    .getNamedQueryResults(creditsConfigQuery);
            ...
            String monthlyRentalRate = purchaseResponse.getMonthlyRentalRate() == null
                    || purchaseResponse.getMonthlyRentalRate().trim()
                            .equals("") ? "0" : ImiDataFormatUtils.roundOffToN(
                    purchaseResponse.getMonthlyRentalRate(), 2);
            monthlyRentalRate = ImiDataFormatUtils.multiply(monthlyRentalRate,
                    creditsMultiplier.toString());
            ...
            String voiceRate = purchaseResponse.getVoicePrice() == null
                    ...
                    : ImiDataFormatUtils.roundOffToN(
                            purchaseResponse.getVoicePrice(), 5);
            voiceRate = ImiDataFormatUtils.multiply(voiceRate,
                    creditsMultiplier.toString());
            voiceRate=ImiDataFormatUtils.roundOffToN(voiceRate, 0);
            resourceMaster.setVoiceInboundPrice(voiceRate);
            String smsRate = purchaseResponse.getSmsRate() == null
                    || purchaseResponse.getSmsRate().trim().equals("") ? "0"
                    : ImiDataFormatUtils.roundOffToN(
                            purchaseResponse.getSmsRate(), 5);
            smsRate = ImiDataFormatUtils.multiply(smsRate,
                    creditsMultiplier.toString());
    ...
    public void updateChannelAssetsAllocation(ResourceMaster resourceMaster,
        ...
        try {
            ...
            channelAssetsAllocation.setChannelId(Integer.parseInt(String
                    .valueOf(resourceMaster.getChannel())));
    ...
    public Purchase updatePurchase(PurchaseResponse purchaseResponse,
        ...
        try {
            purchase.setMonthlyRentalRate(resourceMaster
                    .getMonthlyRentalPrice());
            Provider provider = providerService.getProviderById(resourceMaster
                    .getProviderId());
            Country country = countryDao.getCountryByIso(resourceMaster
                    .getCountryIso());
    ...
    public PurchaseHistory updatePurchasehistory(Purchase purchase) {
        ...
        try {
            ...
            purchasehistory.setMonthlyRentalRate(purchase
                    .getMonthlyRentalRate());
            purchasehistory.setNumber(purchase.getNumber());
            purchasehistory.setNumberType(purchase.getNumberType());
            purchasehistory.setNumberProviderCountry(purchase
                    .getNumberProviderCountry());
            purchasehistory
    ...
    public void provisionData(String number,
            ApplicationResponse applicationResponse) {
        try {
            PurchaseHistory purchaseHistory = purchaseHistoryDao
                    .getPurchaseHistoryByNumber(number);
            Provisioning provisioning = new Provisioning();
            provisioning.setSmsFallbackMethod(applicationResponse
                    .getSmsFallbackMethod());
            provisioning.setSmsFallbackUrl(applicationResponse
                    .getSmsFallbackUrl());
            provisioning.setSmsMethod(applicationResponse.getSmsMethod());
            provisioning.setSmsStatusCallback(applicationResponse
                    .getSmsStatusCallback());
            provisioning.setSmsUrl(applicationResponse.getSmsUrl());
            provisioning.setStatusCallBack(applicationResponse
                    .getStatusCallback());
            provisioning.setStatusCallbackMethod(applicationResponse
                    .getStatusCallbackMethod());
            provisioning.setVoiceFallbackMethod(applicationResponse
                    .getVoiceFallbackMethod());
            provisioning.setVoiceFallbackUrl(applicationResponse
                    .getVoiceFallback());
            provisioning.setNumber(number);
            provisioning
                    .setVoiceUrl(applicationResponse.getVoiceUrl() == null ? applicationResponse
                            .getTrunkSid() : applicationResponse.getVoiceUrl());
    ...
    public ResourceMaster getResourceMasterByNumberAndProvider(String number,
            Provider provider) {
        ResourceMaster resourceMaster = null;
        try {
            resourceMaster = resourceMasterDao.getResourceByNumber(number,
                    provider.getId());
        } catch (Exception e) {
            LOG.error(ImiDataFormatUtils.getStackTrace(e));
        }
    ...
    public void deleteChannelAssetsAllocation(ResourceMaster resourceMaster) {
        try {
            ChannelAssetsAllocation channelAssetsAllocation = channelAssetsAllocationDao
                    .getChannelAssetsAllocationByAssetId(resourceMaster
                            .getResourceId());
            channelAssetsAllocationDao
                    .deleteChannelAssetsAllocation(channelAssetsAllocation);
        } catch (Exception e) {
            LOG.error(ImiDataFormatUtils.getStackTrace(e));
        }
    }

    public void updateNewNumbersReleased(ResourceMaster resourceMaster,
            String clientkey) {
        try {
            ...
            numbersReleased.setCreatedOn(ImiDataFormatUtils
                    .fromDateToTimeStamp(resourceMaster.getCreatedOn()));
            numbersReleased.setMonthlyRental(resourceMaster
                    .getMonthlyRentalPrice());
            numbersReleased.setServiceCode(resourceMaster.getServiceCode());
            numbersReleased.setReleasedOn(ImiDataFormatUtils
                    .getCurrentTimeStamp());
    ...
    public void updateCreditsConfigResource(String number, String uuid,
            String voiceRateCredits) throws ClientProtocolException,
            IOException {
        try {
            String startdate = ImiDataFormatUtils.getCurrentDate("yyyy-MM-dd");
            Date endDate = ImiDataFormatUtils.getDateFromString("9999-12-31",
                    "yyyy-MM-dd");
            long noOfDays = ImiDataFormatUtils.getDifferenceInDaysBetweenDates(
                    new Date(), endDate);
            String resourceRequest = creditsConfigResourceRequest
                    .replace("{number}", number).replace("{teamuuid}", uuid)
            ...
            LOG.info("Made call to credits config" + "authkey : "
                    + creditsResourceAuthkey + " key : " + uuid
                    + "  request : " + resourceRequest);
            GenericRestResponse restResponse = ImiHttpUtil
                    .defaultHttpPostHandler(resourceUrl, requestBody, ContentType.APPLICATION_FORM_URLENCODED.getMimeType());;
            LOG.info("Response from Credits Resource is "
