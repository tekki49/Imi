...
public class SubAccountController {
	...
	private static final Logger LOG = Logger.getLogger(SubAccountController.class);
	...
	public SubAccountDetails getSubAccountDetails(@PathVariable("friendlyName") final String friendlyName,
		...
		SubAccountDetails subAccountDetails = subAccountService.getSubAccountDetailsByFriendlyName(friendlyName,
				provider, userid);
		return subAccountDetails;
	}

	@RequestMapping(value = "/create/subAccount/trunksip/{masterTrunkSid}", method = RequestMethod.POST)
	public String createNewSubAccount(@PathVariable("masterTrunkSid") final String masterTrunkSid,
		...
		SubAccountDetails subAccountDetails = twilioFactoryImpl.createSubAcctDetailsWithTrunk(providerObj, userid,
				clientId, masterTrunkSid);
		Map<String, String> response = new HashMap<String, String>();
		if (subAccountDetails != null) {
			response.put("status", "OK");
			response.put("code", "0");
			response.put("message", "success");
			response.put("detailedMessage", "Sub account for client" + userid + " created successfully");
