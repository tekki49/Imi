...
public class NumberSearchController {
	...
	public NumberResponse numberSearchResponseByProviderId(@PathVariable("countryIsoCode") final String countryIsoCode,
		...
		if (ImiDataFormatUtils.isNumber(provider)) {
			providerObj = providerService.getProviderById(Integer.parseInt(provider));
		} else {
			providerObj = providerService.getProviderByName(provider);
		}
		try {
			numberResponse = numberSearchService.searchPhoneNumbers(serviceTypeEnum, providerObj, countryIsoCode,
					numberType, pattern, "FIRST", "FIRST", markup);
		} catch (IOException e) {
	...
	public NumberResponse nextNumberSearchResponse(@PathVariable("countryIsoCode") final String countryIsoCode,
			@PathVariable("numberType") final String numberType, @PathVariable("serviceType") final String serviceType,
			@PathVariable("pattern") final String pattern, @PathVariable("nextPlivoIndex") final String nextPlivoIndex,
		...
		try {
			if (pattern.equals("@")) {
				...
				numberResponse = numberSearchService.searchPhoneNumbers(serviceTypeEnum, countryIsoCode, numberType,
						pattern, nextPlivoIndex, nextNexmoIndex, markup);
			}
		} catch (IOException e) {
			LOG.error(e.getMessage());
		}
