...
@Component
public class PurchaseAop implements UrlConstants, ProviderConstants, NumberTypeConstants {
	...
	public PurchaseResponse purchaseTwilioNumber(String number, String numberType, Provider provider, Country country,
			ServiceConstants serviceTypeEnum, Integer userid, Integer clientId, Integer groupid, Integer teamid,
			String clientname, String clientkey, SubAccountDetails subAccountDetails, PurchaseRequest purchaseRequest,
		...
		if (!test) {
			restResponse = ImiHttpUtil.defaultHttpPostHandler(twilioPurchaseUrl, requestBody,
					ImiBasicAuthUtil.getBasicAuthHash(provider), null);
		}
		if (test || restResponse.getResponseCode() == HttpStatus.CREATED.value()) {
			...
			if (("none").equalsIgnoreCase(numberDetails.getAddress_requirements())) {
			...
			purchaseResponse.setMonthlyRentalRate(purchaseRequest.getMonthlyRentalRate());
			...
			ResourceMaster resourceMaster = resourceService.updateResource(purchaseRequest.getNumber(),
					purchaseResponse, serviceTypeEnum, provider, country, userid, clientId, groupid, teamid, clientname,
					clientkey, teamuuid, numberType.toUpperCase());
			resourceService.updateResourceAllocation(resourceMaster, userid, clientId, groupid, teamid, clientname,
					clientkey);
			resourceService.updateChannelAssetsAllocation(resourceMaster, userid, clientId, groupid, teamid, clientname,
					clientkey);
			...
			LOG.error("Exception occured while purchasing number " + number + " from service provider "
					+ provider.getName() + " Response from service provider is : " + restResponse.getResponseBody()
					+ " with response code :" + restResponse.getResponseCode());
			throw InboundRestException.createApiException(InboundApiErrorCodes.INVALID_NUMBER_PURCHASE_EXCEPTION,
					"Error while purchasing number " + number + " from " + provider.getName()
							+ " Response from service provider is " + restResponse.getResponseBody());
		}
	}

	public PurchaseResponse purchaseNexmoNumber(String number, String numberType, Provider provider,
		...
		nexmoPurchaseUrl = nexmoPurchaseUrl.replace("{api_key}", provider.getAuthId())
				.replace("{api_secret}", provider.getApiKey()).replace("{country}", country.getCountryIso())
				.replace("{msisdn}", "" + nexmoNumber);
		GenericRestResponse restResponse = new GenericRestResponse();
		if (!test) {
			restResponse = ImiHttpUtil.defaultHttpPostHandler(nexmoPurchaseUrl, new HashMap<String, String>(),
					ImiBasicAuthUtil.getBasicAuthHash(provider), ContentType.APPLICATION_FORM_URLENCODED.getMimeType());
		}
		...
		if (test || restResponse.getResponseCode() == HttpStatus.OK.value()) {
			if (test || ("200".equals(nexmoPurchaseResponse.getErrorcode())
					&& "success".equals(nexmoPurchaseResponse.getErrorCodeLabel()))) {
				...
				purchaseResponse.setAddressRequired(false);
				ResourceMaster resourceMaster = resourceService.updateResource(purchaseRequest.getNumber(),
						purchaseResponse, serviceTypeEnum, provider, country, userid, clientId, groupid, teamid,
						clientname, clientkey, teamuuid, type);
				resourceService.updateResourceAllocation(resourceMaster, userid, clientId, groupid, teamid, clientname,
						clientkey);
				resourceService.updateChannelAssetsAllocation(resourceMaster, userid, clientId, groupid, teamid,
						clientname, clientkey);
				purchaseResponse.setCountryIso(country.getCountryIso());
				Purchase purchase = resourceService.updatePurchase(purchaseResponse, numberType,
						ImiDataFormatUtils.getAddressRestrictions(purchaseResponse.isAddressRequired(), null),
						resourceMaster);
				resourceService.updatePurchasehistory(purchase);
				return purchaseResponse;
			}
			...
			throw InboundRestException.createApiException(InboundApiErrorCodes.INVALID_PROVIDER_EXCEPTION,
					"Error while purchasing number " + nexmoNumber + " from " + provider.getName()
							+ " Response from service provider is " + restResponse.getResponseBody());
			...
			throw InboundRestException.createApiException(InboundApiErrorCodes.INVALID_NUMBER_PURCHASE_EXCEPTION,
					"Some Error occured while purchasing the number " + nexmoNumber + " please try again"
							+ " Response from service provider is " + restResponse.getResponseBody());
		}
		return purchaseResponse;
	}

	public PurchaseResponse purchasePlivoNumber(String number, String numberType, Provider provider,
			com.imi.rest.dao.model.Country country, ServiceConstants serviceTypeEnum, Integer userid, Integer clientId,
			Integer groupid, Integer teamid, String clientname, String clientkey, PurchaseRequest purchaseRequest,
			String teamuuid) throws ClientProtocolException, IOException {
		if (numberType.equalsIgnoreCase(LANDLINE) || numberType.equalsIgnoreCase(LOCAL)) {
		...
		GenericRestResponse restResponse = new GenericRestResponse();
		if (!test) {
			restResponse = ImiHttpUtil.defaultHttpPostHandler(plivoPurchaseUrl, new HashMap<String, String>(),
		...
		if (test || restResponse.getResponseCode() == HttpStatus.CREATED.value()) {
			if (test || plivoPurchaseResponse.getNumberStatus().get(0).getStatus().equalsIgnoreCase("pending")) {
				purchaseResponse.setAddressRequired(true);
			} else {
				purchaseResponse.setAddressRequired(false);
			}
			if (test || "Success".equalsIgnoreCase(plivoPurchaseResponse.getStatus())) {
			...
			Purchase purchase = resourceService.updatePurchase(purchaseResponse, numberType,
					ImiDataFormatUtils.getAddressRestrictions(purchaseResponse.isAddressRequired(),
							purchaseResponse.isAddressRequired() ? "any" : null),
					resourceMaster);
			resourceService.updatePurchasehistory(purchase);
			...
			LOG.error("Exception occured while purchasing number " + number + " from service provider "
					...
					+ " with response code :" + restResponse.getResponseCode());
			throw InboundRestException.createApiException(InboundApiErrorCodes.INVALID_NUMBER_PURCHASE_EXCEPTION,
					"error occured for number provided." + plivoNumber + " Error :" + plivoPurchaseResponse.getError());
		} else {
			LOG.error("Exception occured while purchasing number " + number + " from service provider "
					+ provider.getName() + " Response from service provider is : " + restResponse.getResponseBody()
					+ " with response code :" + restResponse.getResponseCode());
			throw InboundRestException.createApiException(InboundApiErrorCodes.INVALID_NUMBER_PURCHASE_EXCEPTION,
					"error occured for number provided." + plivoNumber + " Response from Service provider  :"
