...
@Component
public class NumberSearchAop implements UrlConstants, ProviderConstants,
        NumberTypeConstants {
    ...
    public void searchTwilioNumbers(Double forexValue,
            Map<String, TwilioNumberPrice> twilioMonthlyPriceMap,
            Provider provider, ServiceConstants serviceTypeEnum,
            String countryIsoCode, String numberType, String pattern,
            String index, NumberResponse numberResponse, String markup)
        ...
        String servicesString = NumberCapabiltyAop
                .generateTwilioCapabilities(serviceTypeEnum);
        String type = "Local";
        ...
        if (twilioNumberSearchResponse.getResponseCode() == HttpStatus.OK
            ...
            for (Number twilioNumber : twilioNumberList) {
                if (twilioNumber != null) {
                    ...
                    String voiceRateInGBP = ImiDataFormatUtils.forexConvert(
                            forexValue,
                            getHikedPrice(
                                    numberTypePricing.getInboundVoicePrice(),
                                    markup));
                    twilioNumber.setVoiceRate(voiceRateInGBP);
                    String monthlyRentalRateInGBP = ImiDataFormatUtils
                            .forexConvert(
                                    forexValue,
                                    getHikedPrice(numberTypePricing
                                            .getMonthlyRentalRate(), markup));
            ...
            LOG.error("Provider details seem to be improper. Please check account details for "
                    + provider.getName());
        }
    }

    public void searchNexmoPhoneNumbers(Provider provider,
            ...
            String previousNexmoIndex, Double forexValue,
            Map<String, CountryPricing> countryPricingMap, String markup)
            throws ClientProtocolException, IOException {
        ...
        while (recursive) {
            ...
            if (restReponse != null
                ...
                if (serviceTypeEnum.equals(ServiceConstants.VOICE)) {
                    expectedFeatures.add(ServiceConstants.VOICE.toString()
                            .toUpperCase());
                } else if (serviceTypeEnum.equals(ServiceConstants.SMS)) {
                    expectedFeatures.add(ServiceConstants.SMS.toString()
                            .toUpperCase());
                                        ...
                                        .forexConvert(
                                                ...
                                                getHikedPrice(
                                                        getNexmoPrice(
                                                                ...
                                                                inboundPrice),
                                                        markup)));
                                getNexmoPrice(countryPricing, numberType,
                                        "inboundRateInEuros");
                            } else if (numberType.equalsIgnoreCase(MOBILE)) {
                                ...
                                nexmoNumber.setVoiceRate(ImiDataFormatUtils
                                        .forexConvert(
                                                ...
                                                getHikedPrice(
                                                        ...
                                                        markup)));
                                nexmoNumber
                                        .setMonthlyRentalRate(ImiDataFormatUtils
                                                .forexConvert(
                                                        forexValue,
                                                        getHikedPrice(
                                                                getNexmoPrice(
                ...
                recursive = false;
                LOG.error("Exception occured while searching for service provider "
                        + provider.getName() + " url is " + url);
            }
        }
    }

    public void searchPlivoPhoneNumbers(Provider provider,
            ServiceConstants serviceTypeEnum, String countryIsoCode,
            String numberTypePlivoApi, String pattern,
            List<Number> numberSearchList, int offset,
            NumberResponse numberResponse, Double forexValue,
            String numberType, String markup) throws ClientProtocolException,
            IOException {
        String plivioPhoneSearchUrl = PLIVO_PHONE_SEARCH_URL;
        plivioPhoneSearchUrl = plivioPhoneSearchUrl
                .replace("{auth_id}", provider.getAuthId())
                .replace("{country_iso}", countryIsoCode)
                .replace("{type}", numberTypePlivoApi)
            ...
            if (restResponse.getResponseCode() == HttpStatus.OK.value()) {
                ...
                if (numberResponseFromPlivo.getMeta() != null
                    ...
                    if (numberSearchList.size() < PLIVO_API_CALL_THRESHOLD) {
                        ...
                        nextPlivoIndex = ""
                                + (numberResponseFromPlivo.getMeta()
                                        .getOffset() + numberResponseFromPlivo
                                        .getMeta().getLimit());
    ...
    private String getHikedPrice(String input, String markup) {
        return ImiDataFormatUtils.getHikedPrice(input,
                markup == null ? defaultMarkup : markup);
    }

    private String getNexmoPrice(CountryPricing countryPricing,
            String numberType, String priceType) {
