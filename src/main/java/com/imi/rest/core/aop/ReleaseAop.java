...
public class ReleaseAop implements UrlConstants {
    ...
    public void releaseTwilioNumber(String number, Provider provider,
            String countryIsoCode, ApplicationResponse incomingPhoneNumber,
            SubAccountDetails subAccountDetails, String clientkey)
            throws IOException {
        if (incomingPhoneNumber == null || subAccountDetails == null) {
            LOG.error("Number requested " + number
                    + " does not belong to your " + provider.getName()
                    + "Account");
        ...
        twilioReleaseurl = twilioReleaseurl.replace("{auth_id}",
                subAccountDetails.getSid()).replace("{IncomingPhoneNumberSid}",
        ...
        if (response.getResponseCode() != HttpStatus.NO_CONTENT.value()) {
            ...
            ResourceMaster resourceMaster = resourceService
                    .getResourceMasterByNumberAndProvider(number, provider);
    ...
    public void releaseNexmoNumber(String number, Provider provider,
            String countryIsoCode, Number numberDetails, String clientkey)
        ...
        if (restResponse.getResponseCode() == HttpStatus.OK.value()) {
            ...
            resourceService.deleteResourceMaster(resourceMaster);
                ...
                .value()) {
            LOG.error("Invalid provider authentication details for "
                    + provider.getName()
                    + " response from service provider is "
                    + restResponse.getResponseBody() + " response code is "
            ...
            throw InboundRestException.createApiException(
                ...
                .value()) {
            LOG.error("Invalid details while releasing the number " + number
            ...
            throw InboundRestException.createApiException(
                    InboundApiErrorCodes.NUMBER_PROVIDER_ASSOCIATION_EXCEPTION,
                    "Number " + number
                            + " does not belong to Nexmo master account.Response from Nexmo is"
                            + restResponse.getResponseBody());
        } else {
            LOG.error("Invalid details while releasing the number " + number
                    + " for " + provider.getName()
    ...
    public void releasePlivoNumber(String number, Provider provider,
            String countryIsoCode, Number numberDetails, String clientkey)
            throws ClientProtocolException, IOException {
        ...
        GenericRestResponse restResponse = ImiHttpUtil
                .defaultHttpDeleteHandler(plivioReleaseurl,
