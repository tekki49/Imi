...
public class PlivoFactoryImpl
    ...
    public void releaseNumber(String number, Provider provider,
        ...
        plivioReleaseurl = plivioReleaseurl.replace("{number}", plivioNumber)
                .replace("{auth_id}", provider.getAuthId());
        String response = HttpUtil.defaultHttpDeleteHandler(plivioReleaseurl,
    ...
    public PurchaseResponse purchaseNumber(String number, String numberType,
        ...
        searchPhoneNumbers(provider, null, country.getCountryIso(), numberType,
                number, "0", numberResponse);
        String response = HttpUtil.defaultHttpPostHandler(plivoPurchaseUrl,
                new HashMap<String, String>(),
                BasicAuthUtil.getBasicAuthHash(provider.getAuthId(),
                        provider.getApiKey()),
                ContentType.APPLICATION_JSON.getMimeType());
        Number numberDetails = numberResponse.getObjects() == null ? null
                : numberResponse.getObjects().get(0);
        PlivoPurchaseResponse plivoPurchaseResponse = ImiJsonUtil
                .deserialize(response, PlivoPurchaseResponse.class);
        if (plivoPurchaseResponse.getError() != null) {
            throw new ImiException("error occured for number provided " + number
    ...
    private ApplicationResponse createApplication(
        ...
        try {
            ...
            String response = HttpUtil.defaultHttpPostHandler(
                    plivoCreateApplicationurl, requestBody,
                    BasicAuthUtil.getBasicAuthHash(provider.getAuthId(),
                            provider.getApiKey()),
                    ContentType.APPLICATION_JSON.getMimeType());
    ...
    private ApplicationResponse getApplication(String appId,
            Provider provider) {
        String plivoApplicationUpdateUrl = PLIVO_APPLICATION_UPDATE_URL;
        plivoApplicationUpdateUrl = plivoApplicationUpdateUrl
                .replace("{auth_id}", provider.getAuthId())
        ...
        try {
            String response = HttpUtil.defaultHttpGetHandler(
                    plivoApplicationUpdateUrl, BasicAuthUtil.getBasicAuthHash(
                            provider.getAuthId(), provider.getApiKey()));
