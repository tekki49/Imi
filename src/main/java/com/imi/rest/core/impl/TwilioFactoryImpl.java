...
public class TwilioFactoryImpl
	...
	public void searchPhoneNumbers(Provider provider, ServiceConstants serviceTypeEnum, String countryIsoCode,
		...
		numberSearchAop.searchTwilioNumbers(forexValue, twilioMonthlyPriceMap, provider, serviceTypeEnum,
				countryIsoCode, numberType, pattern, index, numberResponse, markup);
	}
	...
	public PurchaseResponse purchaseNumber(String number, String numberType, Provider provider, Country country,
			ServiceConstants serviceTypeEnum, Integer userid, Integer clientId, Integer groupid, Integer teamid,
			String clientname, String clientkey, PurchaseRequest purchaseRequest, String teamuuid)
					throws ClientProtocolException, IOException {
		...
		SubAccountDetails subAccountDetails = createSubAccount("" + userid, provider);
		return purchaseAop.purchaseTwilioNumber(number, numberType, provider, country, serviceTypeEnum, userid,
				clientId, groupid, teamid, clientname, clientkey, subAccountDetails, purchaseRequest, teamuuid);
	}

	public void releaseNumber(String number, Provider provider, String countryIsoCode, Integer userid, Integer clientId,
			Integer groupid, Integer teamid, String clientname, String clientkey) throws IOException {
		// get the subaccount details
		SubAccountDetails subAccountDetails = getSubAccountDetails("" + userid, provider);
		if (subAccountDetails == null) {
			String message = "No associated sub account created with the twilio for the client id " + userid;
			LOG.error(message);
			throw InboundRestException
					.createApiException(InboundApiErrorCodes.TWILIO_SUBACCOUNT_USER_ASSOCIATION_EXCEPTION, message);
		}
		String twilioNumber = "+" + (number.trim().replace("+", ""));
		ApplicationResponse incomingPhoneNumber = getIncomingPhoneNumber(twilioNumber, provider, subAccountDetails);
		releaseAop.releaseTwilioNumber(twilioNumber, provider, countryIsoCode, incomingPhoneNumber, subAccountDetails,
				clientkey);
	}

	public SubAccountDetails createSubAcctDetailsWithTrunk(Provider provider, int userid, int clientId,
			String masterTrunkSid) throws JsonParseException, JsonMappingException, IOException {
		SubAccountDetails subAccountDetails = createSubAccount("" + userid, provider);
		SipTrunk masterSipTrunk = getSipTrunkDetailsByTrunkSid(provider.getAuthId(), provider.getApiKey(),
				masterTrunkSid);
		List<OriginationUrl> masterOriginatingUrls = getOriginatingUrlDetails(masterSipTrunk.getSid(),
				provider.getAuthId(), provider.getApiKey());
		List<IpAccessControlList> masterAccessControlLists = getIpAccessControlList(masterSipTrunk.getSid(),
				provider.getAuthId(), provider.getApiKey());
		...
		subAccountSipTrunk = createSipTrunkDetailsForSubAccount(masterSipTrunk.getFriendly_name(), subAccountSid,
		...
		for (OriginationUrl originatingUrl : masterOriginatingUrls) {
			createOriginatingUrlDetails(subAccountSipTrunk.getSid(), subAccountSid, subAccountAuthToken,
					originatingUrl);
		}
		for (IpAccessControlList ipAccessControlList : masterAccessControlLists) {
			List<IpAddress> masterIpAddressList = getIpAddress(provider.getAuthId(), provider.getApiKey(),
					ipAccessControlList.getSid());
			IpAccessControlList subAccountIpAccessList = createIpAccessControlList(
					ipAccessControlList.getFriendly_name(), subAccountSid, subAccountAuthToken);
			for (IpAddress ipAddress : masterIpAddressList) {
				createIpAddress(ipAddress.getFriendly_name(), ipAddress.getIp_address(), subAccountSid,
						subAccountAuthToken, subAccountIpAccessList.getSid());
			}
			associateSipTrunkWithIpAccessControlList(subAccountSipTrunk.getSid(), subAccountIpAccessList.getSid(),
	...
	public ApplicationResponse updateNumber(String number, ApplicationResponse applicationResponsetoModify,
			Provider provider, Integer userid, Integer clientId, Integer groupid, Integer teamid, String clientname,
		...
		List<SipTrunk> sipTrunkList = getSipTrunkDetailsBySubAccountSid(subAccountDetails.getSid(),
				subAccountDetails.getAuth_token());
		...
		if (incomingPhoneNumber.getCapabilities().get("voice") != null
			...
			if (subAcctSipTrunk != null) {
				applicationResponsetoModify.setTrunkSid(subAcctSipTrunk.getSid());
			} else {
				applicationResponsetoModify.setTrunkSid(null);
			}
		}
		...
		return updateNumberAop.updateTwilioNumber(number.trim().replace("+", ""), applicationResponsetoModify, provider,
				incomingPhoneNumber, userid, clientId, groupid, teamid, clientname, clientkey, subAccountDetails);
	}

	// TODO Customer SId must be obtained from the address response and
	// integrated with this method to replace provider AuthID
	private ApplicationResponse getIncomingPhoneNumber(String number, Provider provider,
			SubAccountDetails subAccountDetails) throws ClientProtocolException, IOException {
		number = number.replace("+", "");
		String twilioNumberGetUrl = TWILIO_PURCHASED_NUMBER_URL;
		ApplicationResponse applicationResponse = null;
		twilioNumberGetUrl = twilioNumberGetUrl.replace("{auth_id}", subAccountDetails.getSid())
				.replace("{FriendlyName}", number);
		LOG.info("Making request with Twilio for retrieving the number " + number + " The requested url is "
				+ twilioNumberGetUrl);
		...
		if (restResponse.getResponseCode() == HttpStatus.OK.value()) {
			LOG.info("Response from twilio for getting purchsed phone number details for number " + number + " is"
			...
			if (incomingPhoneNumbers != null && incomingPhoneNumbers.getObjects().size() > 0) {
				loop: for (ApplicationResponse response : incomingPhoneNumbers.getObjects()) {
					if (response.getFriendlyName().equalsIgnoreCase(number)) {
		...
		return applicationResponse == null ? null
				: applicationResponse.getFriendlyName().equals(number) ? applicationResponse : null;
	}

	public ApplicationResponse getIncomingPhoneNumberBySid(String numberSid, String subAccountSid, Provider provider)
			throws ClientProtocolException, IOException {
		String url = TWILIO_RELEASE_URL;
		url = url.replace("{auth_id}", subAccountSid).replace("{IncomingPhoneNumberSid}", numberSid);
		ApplicationResponse incomingPhoneNumber = null;
		GenericRestResponse restResponse = ImiHttpUtil.defaultHttpGetHandler(url,
				ImiBasicAuthUtil.getBasicAuthHash(provider));
		if (restResponse.getResponseCode() == HttpStatus.OK.value()) {
			incomingPhoneNumber = ImiJsonUtil.deserialize(restResponse.getResponseBody(), ApplicationResponse.class);
		}
		return incomingPhoneNumber;
	}

	public SubAccountDetails getSubAccountDetailsBySid(String subAccountSid, Provider provider)
			throws ClientProtocolException, IOException {
	...
	public List<Address> getAddressOfCustomer(String customerName, Provider provider, Integer userid,
			SubAccountDetails customerAccountDetails) throws ClientProtocolException, IOException {
		...
		addressGetUrl = addressGetUrl.replace("{auth_id}", accountSid).concat("?CustomerName=" + customerName);
	...
	public Address createNewAddressOfCustomer(Customer customer, Provider provider, Integer userid)
			throws ClientProtocolException, IOException {
		SubAccountDetails customerAccountDetails = getSubAccountDetails("" + userid, provider);
		List<Address> addressList = getAddressOfCustomer(customer.getCustomer(), provider, userid,
		...
		if (customer.getCountryIso() == null) {
			...
			loop: for (Address address2 : addressList) {
				if (address2.getIso_country().equalsIgnoreCase(customer.getCountryIso())) {
					address = address2;
					break loop;
				}
			}
		...
		if (address == null) {
			...
			addressPostUrl = TWILIO_ADDRESS_BY_SID_URL.replace("{auth_id}", accountSid).replace("{AddressSid}",
					address.getSid());
		}
		...
		if (restResponse.getResponseCode() == HttpStatus.OK.value()
			...
			String message = "Exception while updating the customer address. Error response from Twilio "
					+ restResponse.getResponseBody();
			throw InboundRestException.createApiException(InboundApiErrorCodes.TWILIO_ADRESS_CREATION_EXCEPTION,
					message);
		}
		return address;
	}
		...
		if (subAccountDetails == null) {
			...
			if (restResponse.getResponseCode() == HttpStatus.OK.value()
				...
				String message = "Exception while creating new SubAccount. Error response from Twilio "
						+ restResponse.getResponseBody();
				throw InboundRestException
						.createApiException(InboundApiErrorCodes.TWILIO_SUB_ACCOUNT_CREATION_EXCEPTION, message);
			}
		}
		return subAccountDetails;
	}
	...
	public List<SipTrunk> getSipTrunkDetailsBySubAccountSid(String authId, String apiKey)
			throws ClientProtocolException, IOException {
		String url = TWILIO_LIST_ALL_SIP_TRUNK_URL;
		GenericRestResponse restResponse = ImiHttpUtil.defaultHttpGetHandler(url,
				ImiBasicAuthUtil.getBasicAuthHash(authId, apiKey));
		SipTrunkResponse sipTrunkResponse = ImiJsonUtil.deserialize(restResponse.getResponseBody(),
				SipTrunkResponse.class);
		return sipTrunkResponse.getTrunks();
	}

	public SipTrunk getSipTrunkDetailsByTrunkSid(String authId, String apiKey, String trunkSid)
			throws ClientProtocolException, IOException {
		String url = TWILIO_SIP_TRUNK_URL;
		url = url.replace("{trunk_sid}", trunkSid);
		GenericRestResponse restResponse = ImiHttpUtil.defaultHttpGetHandler(url,
				ImiBasicAuthUtil.getBasicAuthHash(authId, apiKey));
		SipTrunk sipTrunk = ImiJsonUtil.deserialize(restResponse.getResponseBody(), SipTrunk.class);
		return sipTrunk;
	}

	public SipTrunk createSipTrunkDetailsForSubAccount(String trunkName, String subAccountSid,
			String subAccountAuthToken, SipTrunk masterSipTrunk, int clientId) throws ParseException, IOException {
		String url = TWILIO_LIST_ALL_SIP_TRUNK_URL;
		Map<String, String> requestBody = new HashMap<String, String>();
		requestBody.put("FriendlyName", trunkName);
		if (masterSipTrunk.getDomain_name() != null) {
			requestBody.put("DomainName",
					masterSipTrunk.getDomain_name().replace(".pstn.twilio.com", "").concat("" + clientId)
							+ ".pstn.twilio.com");
		}
		if (masterSipTrunk.getDisaster_recovery_url() != null) {
			requestBody.put("DisasterRecoveryUrl", masterSipTrunk.getDisaster_recovery_url());
		}
		if (masterSipTrunk.getDisaster_recovery_method() != null) {
			requestBody.put("DisasterRecoveryMethod", masterSipTrunk.getDisaster_recovery_method());
		}
		if (masterSipTrunk.getSecure() != null) {
			requestBody.put("Secure", masterSipTrunk.getSecure());
		}
		SipTrunk subAccountSipTrunk = null;
		GenericRestResponse restResponse = ImiHttpUtil.defaultHttpPostHandler(url, requestBody,
				ImiBasicAuthUtil.getBasicAuthHash(subAccountSid, subAccountAuthToken), null);
		if (restResponse.getResponseCode() == org.springframework.http.HttpStatus.CREATED.value()) {
			subAccountSipTrunk = ImiJsonUtil.deserialize(restResponse.getResponseBody(), SipTrunk.class);
		}
		return subAccountSipTrunk;
	}

	public List<OriginationUrl> getOriginatingUrlDetails(String trunkSid, String authId, String authToken)
			throws ClientProtocolException, IOException {
		String url = TWILIO_SIP_TRUNK_ORIGINATING_URL;
		url = url.replace("{trunk_sid}", trunkSid);
		GenericRestResponse restResponse = ImiHttpUtil.defaultHttpGetHandler(url,
				ImiBasicAuthUtil.getBasicAuthHash(authId, authToken));
		OriginatingUrlResponse originatingUrlResponse = ImiJsonUtil.deserialize(restResponse.getResponseBody(),
				OriginatingUrlResponse.class);
		return originatingUrlResponse.getOrigination_urls();
	}

	public void createOriginatingUrlDetails(String trunkSid, String authId, String authToken,
			OriginationUrl originatingUrl) throws ParseException, IOException {
		String url = TWILIO_SIP_TRUNK_ORIGINATING_URL;
		url = url.replace("{trunk_sid}", trunkSid);
		Map<String, String> requestBody = new HashMap<String, String>();
		// need to check
		...
		OriginationUrl originationUrl = ImiJsonUtil.deserialize(restResponse.getResponseBody(), OriginationUrl.class);
	}

	public List<IpAccessControlList> getIpAccessControlList(String trunkSid, String authId, String authToken)
			throws ClientProtocolException, IOException {
		...
		IpAccessControlListResponse ipAccessControlListResponse = ImiJsonUtil
				.deserialize(restResponse.getResponseBody(), IpAccessControlListResponse.class);
		System.out.println(restResponse.getResponseBody());
		return ipAccessControlListResponse.getIp_access_control_lists();
	}

	public IpAccessControlList createIpAccessControlList(String friendlyName, String authId, String authToken)
			throws ClientProtocolException, IOException {
		...
		GenericRestResponse restResponse = ImiHttpUtil.defaultHttpPostHandler(url, requestBody,
				ImiBasicAuthUtil.getBasicAuthHash(authId, authToken), null);
		IpAccessControlList ipAccessControlList = ImiJsonUtil.deserialize(restResponse.getResponseBody(),
				IpAccessControlList.class);
		return ipAccessControlList;
	}

	public void associateSipTrunkWithIpAccessControlList(String trunkSid, String ipAccessControlSid, String authId,
			String authToken) throws ClientProtocolException, IOException {
		String url = TWILIO_SIP_TRUNK_IP_ACCESS_CONTROL_LIST_ASSOC_URL;
		url = url.replace("{trunk_sid}", trunkSid);
		Map<String, String> requestBody = new HashMap<String, String>();
	...
	public List<IpAddress> getIpAddress(String authId, String authToken, String ipAccessControlSid)
			throws ClientProtocolException, IOException {
		String url = TWILIO_IP_ADDRESS_URL;
		url = url.replace("{access_control_list_sid}", ipAccessControlSid).replace("{auth_id}", authId);
		GenericRestResponse restResponse = ImiHttpUtil.defaultHttpGetHandler(url,
				ImiBasicAuthUtil.getBasicAuthHash(authId, authToken));
		IpAddressResponse ipAddressResponse = ImiJsonUtil.deserialize(restResponse.getResponseBody(),
				IpAddressResponse.class);
		return ipAddressResponse.getIp_addresses();
	}

	public IpAddress createIpAddress(String friendlyName, String ipAddress, String authId, String authToken,
			String ipAccessControlSid) throws ClientProtocolException, IOException {
		String url = TWILIO_IP_ADDRESS_URL;
		url = url.replace("{access_control_list_sid}", ipAccessControlSid).replace("{auth_id}", authId);
		Map<String, String> requestBody = new HashMap<String, String>();
		requestBody.put("FriendlyName", friendlyName);
