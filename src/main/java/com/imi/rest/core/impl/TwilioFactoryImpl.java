...
public class TwilioFactoryImpl
	...
	public void searchPhoneNumbers(Provider provider, ServiceConstants serviceTypeEnum, String countryIsoCode,
		...
		twilioNumberSearchResponse = ImiHttpUtil.defaultHttpGetHandler(twilioPhoneSearchUrl,
				ImiBasicAuthUtil.getBasicAuthHash(provider.getAuthId(), provider.getApiKey()));
	...
	public Set<com.imi.rest.model.Country> importCountriesByUrl(Provider provider)
		...
		String authHash = ImiBasicAuthUtil.getBasicAuthHash(provider.getAuthId(), provider.getApiKey());
	...
	public PurchaseResponse purchaseNumber(String number, String numberType, Provider provider, Country country,
		...
		GenericRestResponse response = ImiHttpUtil.defaultHttpPostHandler(twilioPurchaseUrl, requestBody,
				ImiBasicAuthUtil.getBasicAuthHash(provider.getAuthId(), provider.getApiKey()), null);
		if (response.getResponseCode() == HttpStatus.CREATED.value()) {
			...
			TwilioNumberPrice twilioPrice = (twilioMonthlyPriceMap == null
					? getTwilioPricing(country.getCountryIso(), provider).get(searchKey)
	...
	public void releaseNumber(String number, Provider provider, String countryIsoCode)
		...
		GenericRestResponse twilioAccountRestResponse = ImiHttpUtil.defaultHttpGetHandler(TWILIO_ACCOUNT_URL,
				ImiBasicAuthUtil.getBasicAuthHash(provider.getAuthId(), provider.getApiKey()));
		String accountSid = null;
		if (twilioAccountRestResponse.getResponseCode() == HttpStatus.OK.value()) {
			TwilioAccountResponse twilioAccountResponse = ImiJsonUtil
					.deserialize(twilioAccountRestResponse.getResponseBody(), TwilioAccountResponse.class);
			if (twilioAccountResponse.getAccounts() != null && twilioAccountResponse.getAccounts().size() > 0) {
				accountSid = twilioAccountResponse.getAccounts().get(0).getSid();
			}
		}
		// TODO: need to get IncomingPhoneNumberSid
		String incomingPhoneNumberSid = number;
		String twilioReleaseurl = TWILIO_RELEASE_URL;
		twilioReleaseurl = twilioReleaseurl.replace("{AccountSid}", accountSid).replace("{IncomingPhoneNumberSid}",
	...
	private ApplicationResponse getIncomingPhoneNumber(String number, Provider provider)
		...
		ApplicationResponse applicationResponse = new ApplicationResponse();
		GenericRestResponse response = ImiHttpUtil.defaultHttpGetHandler(twilioNumberGetUrl,
				ImiBasicAuthUtil.getBasicAuthHash(provider.getAuthId(), provider.getApiKey()));
		if (response.getResponseCode() == HttpStatus.OK.value()) {
			ApplicationResponseList incomingPhoneNumbers = ImiJsonUtil.deserialize(response.getResponseBody(),
