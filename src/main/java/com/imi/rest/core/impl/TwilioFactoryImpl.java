...
public class TwilioFactoryImpl implements NumberSearch, CountrySearch, PurchaseNumber, UrlConstants, ProviderConstants,
	...
	public void searchPhoneNumbers(Provider provider, ServiceConstants serviceTypeEnum, String countryIsoCode,
		...
		twilioPhoneSearchUrl = twilioPhoneSearchUrl.replace("{country_iso}", countryIsoCode)
				.replace("{services}", servicesString).replace("{pattern}", "*" + pattern.trim() + "*")
				.replace("{type}", type);
		String response = "";
		...
		NumberResponse numberResponseFromTwilo = ImiJsonUtil.deserialize(response, NumberResponse.class);
		...
		for (Number twilioNumber : twilioNumberList) {
			if (twilioNumber != null) {
				...
				String monthlyRentalRateInGBP = DataFormatUtils.forexConvert(
						forexValue, numberTypePricing.getMonthlyRentalRate());
	...
	public void releaseNumber(String number, Provider provider, String countryIsoCode) throws IOException {
		// TODO: need to get IncomingPhoneNumberSid
		String incomingPhoneNumberSid = number;
		String twilioReleaseurl = TWILIO_RELEASE_URL;
		twilioReleaseurl = twilioReleaseurl.replace("{AccountSid}", provider.getAuthId())
				.replace("{IncomingPhoneNumberSid}", incomingPhoneNumberSid);
		String response = HttpUtil.defaultHttpDeleteHandler(twilioReleaseurl, new HashMap<String, String>(),
				BasicAuthUtil.getBasicAuthHash(provider.getAuthId(), provider.getApiKey()), null);
	...
	private ApplicationResponse getIncomingPhoneNumber(String number, Provider provider) {
		...
		try {
			String response = HttpUtil.defaultHttpGetHandler(twilioNumberGetUrl,
					BasicAuthUtil.getBasicAuthHash(provider.getAuthId(), provider.getApiKey()));
			ApplicationResponseList incomingPhoneNumbers = ImiJsonUtil.deserialize(response,
