...
public class NexmoFactoryImpl
	...
	public void searchPhoneNumbers(Provider provider, ServiceConstants serviceTypeEnum, String countryIsoCode,
			String numberType, String pattern, String nexmoIndex, NumberResponse numberResponse, String markup)
		...
		if (numberType.equalsIgnoreCase(MOBILE)) {
			...
			type = MULTI;
		} else if (numberType.equalsIgnoreCase(LANDLINE) || numberType.equalsIgnoreCase(LOCAL)) {
			type = LANDLINE;
		}
		int index = 1;
		...
		searchPhoneNumbers(provider, serviceTypeEnum, countryIsoCode, type, pattern, numberSearchList, index,
				numberResponse, "FIRST", markup);
	...
	void searchPhoneNumbers(Provider provider, ServiceConstants serviceTypeEnum, String countryIsoCode,
			...
			String previousNexmoIndex, String markup) throws ClientProtocolException, IOException {
		// if(numberSearchList.size()>THRESHOLD)return;
		forexValue = forexService.getForexValueByName("EUR_GBP").getValue();
		if (countryPricingMap == null) {
			countryPricingMap = pricingAop.getNexmoPricing(provider);
		}
		numberSearchAop.searchNexmoPhoneNumbers(provider, serviceTypeEnum, countryIsoCode, numberType, pattern,
				numberSearchList, index, numberResponse, previousNexmoIndex, forexValue, countryPricingMap, markup);
	}
	...
	public void releaseNumber(String number, Provider provider, String countryIsoCode, Integer userid, Integer clientId,
		...
		releaseAop.releaseNexmoNumber(nexmoNumber, provider, countryIsoCode, numberDetails, clientkey);
	}
	...
	public BalanceResponse checkBalance(Provider provider) throws ClientProtocolException, IOException {
		...
		nexoAccountBalanceurl = nexoAccountBalanceurl.replace("{api_key}", provider.getAuthId()).replace("{api_secret}",
				provider.getApiKey());
		BalanceResponse balanceResponse = new BalanceResponse();
		GenericRestResponse restResponse = ImiHttpUtil.defaultHttpGetHandler(nexoAccountBalanceurl,
				ImiBasicAuthUtil.getBasicAuthHash(provider));
		if (restResponse.getResponseCode() == HttpStatus.OK.value()) {
			balanceResponse = ImiJsonUtil.deserialize(restResponse.getResponseBody(), BalanceResponse.class);
		}
	...
	public Number getPurchaseNumberDetails(String number, Provider provider)
		...
		if (restResponse.getResponseCode() == HttpStatus.OK.value()) {
			NumberResponse nexmoNumberResponse = ImiJsonUtil.deserialize(
					restResponse.getResponseBody() == null ? "" : restResponse.getResponseBody(), NumberResponse.class);
			loop: for (Number purchasedNumber : nexmoNumberResponse.getObjects()) {
		...
		return numberDetails == null ? null : numberDetails.getNumber().equals(number) ? numberDetails : null;
	}
	...
	public ApplicationResponse updateNumber(String number, String countryIsoCode, ApplicationResponse application,
		...
		return updateNumberAop.updateNexmoNumber(nexmoNumber, countryIsoCode, application, provider, numberDetails,
	...
	public PurchaseResponse purchaseNumber(String number, String numberType, Provider provider,
		...
		return purchaseAop.purchaseNexmoNumber(number, numberType, provider, country, serviceTypeEnum, userid, clientId,
				groupid, teamid, clientname, clientkey, purchaseRequest, teamuuid);
	}
