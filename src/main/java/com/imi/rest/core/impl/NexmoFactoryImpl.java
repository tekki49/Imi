...
public class NexmoFactoryImpl
	...
	public void searchPhoneNumbers(Provider provider, ServiceConstants serviceTypeEnum, String countryIsoCode,
			String numberType, String pattern, String nexmoIndex, NumberResponse numberResponse)
	...
	void searchPhoneNumbers(Provider provider, ServiceConstants serviceTypeEnum, String countryIsoCode,
		...
		NumberResponse nexmoNumberResponse = ImiJsonUtil.deserialize(response, NumberResponse.class);
		if (nexmoNumberResponse == null)
			return;
		List<Number> nexmoNumberList = nexmoNumberResponse == null ? new ArrayList<Number>()
				: nexmoNumberResponse.getObjects() == null ? new ArrayList<Number>() : nexmoNumberResponse.getObjects();
		if (getNexmoPricingResponse() == null) {
			setNexmoPricingResponse(getNexmoPricing(NEXMO_PRICING_FILE_PATH));
		}
		List<CountryPricing> countryPricingList = ImiJsonUtil.deserializeList(nexmoPricingResponse,
		...
		for (Number nexmoNumber : nexmoNumberList) {
			if (nexmoNumber != null) {
				...
				CountryPricing countryPricing = countryPricingMap.get(nexmoNumber.getCountry().toUpperCase());
				String type = "normal";
				if (nexmoNumber.getNumberType().equalsIgnoreCase("mobile-lvn")) {
					nexmoNumber.setType("mobile");
				} else if (nexmoNumber.getNumberType().equalsIgnoreCase("local")) {
					nexmoNumber.setType("landline");
				}
				nexmoNumber.setPriceUnit("EUR");
				nexmoNumber.setMonthlyRentalRate(DataFormatUtils.forexConvert(forexValue,
						countryPricing.getPricing().get(type).get("monthlyRateInEuros")));
				nexmoNumber.setVoiceRate(DataFormatUtils.forexConvert(forexValue,
						countryPricing.getPricing().get(type).get("voiceRateInEuros")));
	...
	public Set<Country> importCountries()
		...
		InputStream in = getClass().getResourceAsStream(NEXMO_PRICING_FILE_PATH);
	...
	public void releaseNumber(String number, Provider provider, String countryIsoCode)
		...
		nexmoReleaseUrl = nexmoReleaseUrl.replace("{api_key}", provider.getAuthId()).replace("{api_secret}",
				provider.getApiKey().replace("{country}", "").replace("{msisdn}", nexmoNumber));
		// TODO handle release response
		String response = HttpUtil.defaultHttpPostHandler(nexmoReleaseUrl, new HashMap<String, String>(),
	...
	public PurchaseResponse purchaseNumber(String number, String numberType, Provider provider,
		...
		String response = HttpUtil.defaultHttpPostHandler(nexmoPurchaseUrl, new HashMap<String, String>(),
				BasicAuthUtil.getBasicAuthHash(provider.getAuthId(), provider.getApiKey()),
				ContentType.APPLICATION_FORM_URLENCODED.getMimeType());
